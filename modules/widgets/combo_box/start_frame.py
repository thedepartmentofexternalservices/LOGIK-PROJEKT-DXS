...
from modules.widgets.combo_box.items_start_frame import combo_box_items_start_frame
...
class WidgetStartFrame(QComboBox):
    def __init__(self, parent=None):
        super().__init__(parent)
        
        # Set object name if needed
        self.setObjectName("template_start_frame")

        # Load items from the imported list
        self.addItems(combo_box_items_start_frame)

        # Set default properties
        self.setCurrentText(self.get_default_start_frame())
        self.setEditable(False)

    def get_widget_parameters(self):
        widget_parameters = {
            "widget_name": "template_start_frame",
            "widget_type": "QComboBox",
            "widget_label_name": "Start Frame: ",
            "widget_default_value": self.get_default_start_frame(),
            "widget_placeholder_value": "",
            "widget_item_values": "items_start_frame.py",
            "widget_read_only": False
        }
        return widget_parameters

    def get_default_start_frame(self):
        """
        ...
        try:
            with open(json_file_path, 'r') as file:
                data = json.load(file)
            start_frame = data.get("Start Frame: ")
            return start_frame
        except:
