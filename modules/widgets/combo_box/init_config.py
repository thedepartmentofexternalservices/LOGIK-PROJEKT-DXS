...
from modules.widgets.combo_box.items_init_config import combo_box_items_init_config
...
class WidgetInitConfig(QComboBox):
    def __init__(self, parent=None):
        super().__init__(parent)
        
        # Set object name if needed
        self.setObjectName("template_init_config")

        # Load items from the imported list
        self.addItems(combo_box_items_init_config)

        # Set default properties
        self.setCurrentText(self.get_default_init_config())
        self.setEditable(False)

    def get_widget_parameters(self):
        widget_parameters = {
            "widget_name": "template_init_config",
            "widget_type": "QComboBox",
            "widget_label_name": "Init Config: ",
            "widget_default_value": self.get_default_init_config(),
            "widget_placeholder_value": "",
            "widget_item_values": "items_init_config.py",
            "widget_read_only": False
        }
        return widget_parameters

    def get_default_init_config(self):
        """
        Reads the JSON file and returns the default init config.
        """
        ...
        try:
            with open(json_file_path, 'r') as file:
                data = json.load(file)
            init_config = data.get("Init Config: ")
            return init_config
        except:
            # Return a default value in case of an error
            return "1920x1080@23976p.cfg"
