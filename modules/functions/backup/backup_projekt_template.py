...
def backup_projekt_template(
    ...
    def generate_backup_filename(filepath):
        ...
        return f"{base}-{date_str}-{the_projekt_name}.json"
...
# -------------------------------------------------------------------------- #
    ...
    # Create the tgt_cfg_template_dir if it doesn't exist
    os.makedirs(
        tgt_cfg_template_dir,
        exist_ok=True
    )

    # Set the source and target cfg preferences
    src_projekt_template = "resources/tmp/current_projekt_template.json"
    tgt_projekt_template = os.path.join(
        tgt_cfg_template_dir,
        f"projekt_template"
    )

    # Archive the PROJEKT template
    print(f"  Archiving PROJEKT template JSON.\n")

    backup_filename = generate_backup_filename(tgt_projekt_template)

    shutil.copy(src_projekt_template, backup_filename)
    
    print(f"  Successfully archived PROJEKT template JSON to:\n")
