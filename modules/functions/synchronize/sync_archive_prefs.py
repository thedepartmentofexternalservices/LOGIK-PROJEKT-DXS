...
def sync_archive_prefs(
    ...
    if os.path.exists(tgt_project_archive_prefs):
        backup_filename = generate_backup_filename(tgt_project_archive_prefs)
        print(f"  * {tgt_project_archive_prefs} exists\n")
        print(f"  * Backing up older archive prefs:\n")
        print(f"  *   {backup_filename}")
        shutil.move(tgt_project_archive_prefs, backup_filename)

    # Copy archive prefs
    shutil.copy(src_project_archive_prefs, tgt_project_archive_prefs)

    # Check if tgt_project_archive_cache_prefs exists and rename if it does
    if os.path.exists(tgt_project_archive_cache_prefs):
        backup_filename = generate_backup_filename(tgt_project_archive_cache_prefs)
        print(f"  * {tgt_project_archive_cache_prefs} exists\n")
        print(f"  * Backing up older archive prefs:\n")
        print(f"  *   {backup_filename}")
        shutil.move(tgt_project_archive_cache_prefs, backup_filename)

    # Copy archive cache prefs
    shutil.copy(src_project_archive_cache_prefs, tgt_project_archive_cache_prefs)
