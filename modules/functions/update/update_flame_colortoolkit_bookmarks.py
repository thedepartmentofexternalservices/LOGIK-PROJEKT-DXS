...
# File Name:        update_flame_colortoolkit_bookmarks.py
# Version:          2.0.0
# Created:          2024-01-19
# Modified:         2024-12-31
...
def update_flame_colortoolkit_bookmarks(
        file_path,
        flame_colortoolkit_bookmarks,
        tgt_projekt_transforms_dir,
    ):
    """
    Update the flame_colortoolkit bookmarks in the specified JSON file.

    Args:
        file_path (str): The path to the JSON file.
        flame_colortoolkit_bookmarks (list): A list of new bookmarks to be added.
    """
    # Read the existing JSON data
    with open(file_path, 'r') as file:
        data = json.load(file)

    # Navigate through the nested structure to find the "utilities" folder section
    sections = data.get("DlBookmark", {}).get("Sections", [])
    for section in sections:
        bookmarks = section.get("Bookmarks", [])
        for bookmark in bookmarks:
            if bookmark.get("Folder") == "projekt directories":
                sub_bookmarks = bookmark.get("Bookmarks", [])
                for sub_bookmark in sub_bookmarks:
                    if sub_bookmark.get("Folder") == "utilities":
                        sub_bookmark["Bookmarks"].extend(flame_colortoolkit_bookmarks)
                        break
...
if __name__ == "__main__":
    ...
    file_path = 'resources/tmp/current_projekt_bookmarks_test.json'

    # Call the function to update the bookmarks
    update_flame_colortoolkit_bookmarks(file_path, flame_colortoolkit_bookmarks, tgt_projekt_transforms_dir)
