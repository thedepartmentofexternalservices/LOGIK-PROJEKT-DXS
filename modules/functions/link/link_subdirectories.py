...
def symlink_subdirectories(
    ...
    target_setups_dir = os.path.abspath(
        os.path.join(
            the_projekts_dir,
            the_projekt_dir,
            "flame",
            "setups",
            the_sanitized_version,
            # "delta"  # replace with the_hostname,
            the_hostname
        )
    )

    print(f"  Linking flame setup directories\n")
    print(f"  from: {source_setups_dir}")
    print(f"  to:   {target_setups_dir}\n")

    if not os.path.isdir(source_setups_dir):
        print(f"  Source setups directory does not exist: {source_setups_dir}")
        return

    os.makedirs(target_setups_dir, exist_ok=True)
    
    for item in os.listdir(source_setups_dir):
        source_path = os.path.join(source_setups_dir, item)
        target_path = os.path.join(target_setups_dir, item)
        
        if os.path.isdir(source_path):
            try:
                if os.path.lexists(target_path):
                    print(f"  Removing existing link: {item}")
                    os.remove(target_path)
                
                os.symlink(source_path, target_path)
                print(f"  Successfully linked: {item} -> {os.path.basename(target_path)}")
            except Exception as e:
                print(f"  Error creating symbolic link for {item}: {e}")

def main():
    parser = argparse.ArgumentParser(description="Set up symbolic links for Flame project directories.")
    parser.add_argument("--the_projekts_dir", required=True, help="Path to the projekts directory")
    parser.add_argument("--the_projekt_dir", required=True, help="Name of the projekt directory")
    parser.add_argument("--the_projekt_flame_dirs", required=True, help="Path to the projekt flame directories")
    parser.add_argument("--the_projekt_flame_dir", required=True, help="Name of the projekt flame directory")
    parser.add_argument("--the_sanitized_version", required=True, help="Sanitized Software version")
    parser.add_argument("--the_hostname", required=True, help="Hostname")
